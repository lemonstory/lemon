...
class onlineList extends controller
{
    ...
    public function action()
    {
        ...
        $onlineListKey = RedisKey::getOnlineListKey($minAge,$maxAge,$page,$len);
        $redisData = $redisobj->get($onlineListKey);
        if ($redisData) {
            $recommendAlbumArr = unserialize($redisData);
        }
        else {
            $albumObj = new Album();
            ...
            if (!empty($recommendAlbumList)) {
                    ...
                    foreach ($recommendAlbumList as $key => $item) {
                        ...
                        if (!empty($recommendAlbumList[$key]['cover'])) {
                            $albumInfo['cover'] = $aliossObj->getImageUrlNg($aliossObj->IMAGE_TYPE_ALBUM,
                                $recommendAlbumList[$key]['cover'], 460, $recommendAlbumList[$key]['cover_time']);
                        }
                        ...
                        $albumAgeLevelStr = $albumObj->getAgeLevelStr($recommendAlbumList[$key]['min_age'],
                            $recommendAlbumList[$key]['max_age']);
                ...
                $recommendAlbumArr['age_level'] = $albumObj->getAgeLevelWithAlbumsFormat($onlineAgeLevelNum,
                    $selectedIndex);
            }
            $redisobj->setex($onlineListKey, self::CACHE_EXPIRE, serialize($recommendAlbumArr));
        }
